...
function getDb(dbName, operation){
    var dbExpries = Date.now()  - config.db.exprieTime;
    if(typeof dbPools[dbName] === "undefined" || dbPools[dbName] === null 
        || !dbPools[dbName].lastOperation || dbExpries <  dbPools[dbName].lastOperation ){
      debug("Connecting to db", dbName);
      if( typeof dbPools[dbName] !== "undefined" && dbPools[dbName].lastOperation && dbExpries <  dbPools[dbName].lastOperation ){
